...
3. **Listing badges only after the listing is confirmed.** A directory's badge is added when its detail page for this plugin loads (see [../50-listings.md](../50-listings.md)). The only exception is dsh.pub, whose badge reports "not listed" by itself until the plugin is listed.
...
**Row 2: directory listings**, dsh.pub first, then others in the order they were confirmed:
...
The README's second row starts with dsh.pub and an HTML comment marking where confirmed listing badges go.
