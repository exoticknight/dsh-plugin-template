...
| `.gitignore`                 | `node_modules/`, `*.tgz`, `.env*`, `.dsh-dev/`, `.dsh-cli/`, `.research/`, `.evolve/`, `lib/*.map`                                                                                                   |                                             |
...
| Install safety               | No `prepare`/`prepack`/`preinstall`/`install`/`postinstall` scripts; runtime deps from the registry without build scripts; peers only `@deepseek-ai/*`; nothing in `files` is `export-ignore`d; no uploaded Release assets (`installing.md`) | `pnpm test` |
| Dev isolation                | `pnpm dev*` and `pnpm smoke` run the DSH CLI via `scripts/dsh.mjs` (isolated homes, verified, no PATH shim; the user's own DSH only after asking) | `pnpm dev:install` |
...
| Release workflow             | tag `v*` → gate → CI → npm OIDC (idempotent) → GitHub Release (generated PR categories and attribution, optional introduction, no uploaded assets); prerelease → `next` | `.github/workflows/release.yml`, `.github/release.yml` |
...
| Docs                         | `README.md` (Chinese) + `README.en.md` link to the plugin's GitHub Releases for version history; `CONTRIBUTING.md`, `SECURITY.md`, `docs/development.md`, `docs/compatibility.md` | |
...
| Local tools | `gh` logged in, pnpm, an isolated DSH CLI from `pnpm dev:cli` (or `DSH_BIN`; see `installing.md`); `npm login` only for the first publish; `red` CLI optional |
