...
每写一个新插件，AI 都要从头摸索一遍 DSH 的插件结构，发布配置也要来回改好几轮。这个模板把这些重复工作一次做好：

| 以前                                                          | 用模板之后                                                                   |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| AI 每次都要重新摸索 `dsh.bundle`、`cordis.patch.yml`、client 打包格式 | 可运行的 host / client 骨架和契约测试已经就位，AI 直接写业务                  |
| 从 GitHub 或 dsh.pub 安装时报缺少入口文件                      | `lib/` 入库，pre-commit 自动重建，CI 拦截过期产物                             |
| 发布靠手动 `npm publish`，tag、版本号、Release 经常对不上      | 推一个 tag，自动完成校验 → CI → npm → GitHub Release                          |
| npm token 放在仓库 secrets 里，担心泄露                       | Trusted Publishing（OIDC），自动 provenance，仓库里没有 token                 |
| Windows 与 Linux 构建结果不一致                               | 统一 LF 换行，CI 同时跑 Ubuntu 和 Windows                                     |
| 不知道该把插件提交到哪些目录，badge 链接贴错                   | 插件目录清单、提交流程、badge 规则，全部写成手册                             |
| 发布失败后不知道该重跑、删 tag 还是发新版本                    | 每个失败点都有对应的恢复步骤                                                 |
